<script>
    ...
    import { updateAppointmentQuestion } from "$providers/actions/kchannel/appointment/form";
    ...
    let loadAnUpdateFormQuestionState = {
    ...
    const onLoadAnUpdatedFormQuestion = async () => {
		try {
			loadAnUpdateFormQuestionState.errMsg = "";
			loadAnUpdateFormQuestionState.loading = true;
			loadAnUpdateFormQuestionState.loaded = false;
            options.forEach((value, index) => {
                answers[index] = value;
            });
			const res = await updateAppointmentQuestion.load({
                id: id,
                question: question,
                type: fieldType,
                answer: answers,
			...
			loadAnUpdateFormQuestionState.updatedFormQuestion = res.data;
			loadAnUpdateFormQuestionState.loaded = true;
			dispatch("updated");
		} catch (err) {
			loadAnUpdateFormQuestionState.errMsg = err.message;
		} finally {
			loadAnUpdateFormQuestionState.loading = false;
            console.log(loadAnUpdateFormQuestionState);
		}
	};
</script>
...
<div class="flex items-center justify-center w-full h-full">
    <div class=" w-[50%] bg-white rounded-lg shadow p-[1vw] min-h-1/2">
        <form class="container mx-auto p-4" on:submit|preventDefault={onLoadAnUpdatedFormQuestion}>
            ...
            {#if fieldType == "Check Box" | fieldType == "Select"}
                <div transition:animate class="p-3 border rounded-lg">
                    <button on:click={addOption} type="button" class="flex text-sm text-[#004E80] items-center hover:underline">
            ...
            <div class="flex items-center justify-end">
                ...
                <button type="submit" class="bg-[#004E80] hover:bg-[rgb(27,54,71)] text-white py-1 px-2 rounded-lg duration-100">Update</button>
            </div>
        </form>
