        ...
        form.questions.forEach((q) => {
          ...
          if (q.type !== "header") {
            if (q.type === "checkbox" || q.type === "CHECKBOXES") {
              answers[qId] = [];
            } else if (q.type === "LINEAR_SCALE" || q.type === "NUMBER") {
              answers[qId] = null;
            } else {
              answers[qId] = "";
            }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const formattedAnswers = Object.keys(answers)
        ...
        .map((questionId) => {
          const question = form.questions.find(q => (q._id || q.id) === questionId);
          const answerValue = answers[questionId];
          
          // Determine the proper value type based on question type
          let value;
          if (Array.isArray(answerValue)) {
            value = answerValue.join(", ");
          } else if (question?.type === "LINEAR_SCALE" || question?.type === "NUMBER") {
            // Keep numeric values as numbers
            value = typeof answerValue === 'number' ? answerValue : Number(answerValue);
          } else {
